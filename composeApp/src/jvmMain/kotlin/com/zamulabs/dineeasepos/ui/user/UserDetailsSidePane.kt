...
fun UserDetailsSidePane(
    ...
    Column(modifier = modifier.padding(horizontal = 16.dp)) {
        ...
        if (user == null) {
            Text("Select a user to see details", color = MaterialTheme.colorScheme.outline)
        } else {
            RowItem("Name", user.name)
            RowItem("Role", user.role)
            RowItem("Status", if (user.active) "Active" else "Inactive")
